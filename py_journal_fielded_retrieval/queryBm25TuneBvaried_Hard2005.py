# Tune B for title and body. We are using custom BM25F scripting not the ES BM25F
# Therefore we don't need change the ES BM25 parameters
...
topPrefix = "topBm25fTuneB_Hard2005"
k = 1.2
result_size = 1000
...
def es_search(p_alpha, p_size, p_k1, p_bt, p_bb, p_avgTitle, p_avgBody, p_df, p_query):
    resultstring = ''
    print("processing alpha: {} bt: {} bb: {} qNum: {}".format(p_alpha, p_bt, p_bb, p_query["queryNumber"]))
...
print("Opened index {0} --> b: {1}, k1: {2}".format(indexName, 0.75, k))
...
for bt in xrange(0, 101, 5):
    for bb in xrange(0, 101, 5):
        for t_alpha in xrange(0, 11, 1):
            ...
            func = partial(es_search, float(t_alpha)/10, result_size, k, float(bt) / 100, float(bb) / 100,
                           avgTitle, avgBody, terms_df)
