...
#top and result file legend
#1 use all fields
#2 use terms in title, alias From Matching Title, and categories From Matching Title
#3 use original query only
#4 use original query and alias from matching title
#5 use original query and terms in title
#6 use original query and categories From Matching Title
...
if kb == "wiki":
    ...
    resultFile = "trecEval_ExpWikiHealth_" + weightScheme + "_rank" + str(rankTreshold).zfill(2) + '_score' + str(scoreTreshold).zfill(2) + '_len' + str(maxLength) + "_Clef2016_" + str(expScheme) + ".csv"
elif kb == "pubmed":
...
trecResults = commands.getoutput(trecPath + 'trec_eval -q -m map -m P.10 -m ndcg_cut.10,1000 -m bpref -m relstring.10 ' + qrelPath + " " + topPath + topFileName)
